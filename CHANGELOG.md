...
- Replace `getEntries` with `getEntryHashes` and `getEntriesByIds` so that callers can cache entry content by hash
  and retrieve only changed entries

## [1.0.0] - 2026-02-27

- Re-release as 1.0.0 without changes

## [0.30.0] - 2026-02-26

### Changed

- Rename commit result field to `commitHash` to better represent actual data returned
...
- Change model field name to be use case agnostic
